...
class MetroMap {
    ...
    addNode(name, x, y, passLine, xSpace = 7, ySpace = 11) {
        ...
        let node = new Vertex(name, x, y, passLine, xSpace, ySpace);
        this.vertexMap.set(name, node);
    ...
    //通过Dijkstra算法查找起点站到终点站的最短路径
    searchShortestPath(startStation, endStation) {
        let openList = new Map(); //存放已经拓展但还未求得最短路径值的节点
        let closeList = new Map(); //存放已经得到最短路径值的节点
        openList.set(startStation, 0);
        ...
        while (true) {
            //openList中没有节点了仍未搜索到终点站的节点，则搜索失败
            if (openList.size == 0) {
                return false;
            }

            let minNode, minValue = Infinity;
            for (let [node, value] of openList) {
                if (value < minValue) {
                    [minNode, minValue] = [node, value];
                }
            }

            openList.delete(minNode); //在openList中删除当前节点
            closeList.set(minNode, minValue); //将当前节点加入到closeList中
            ...
            for (let [node, value] of this.vertexMap.get(minNode).edges) {
                if (!closeList.has(node)) {
                    //如果当前节点在openList中，则对其最短距离进行更新,否则将其添加到openList中
                    if (openList.has(node)) {
                        if (value + minValue < openList.get(node)) {
                            openList.set(node, value + minValue);
    ...
    //生成换乘指南
    getTransferInfo() {
        let info = `<strong>${this.transferArray[0]}->${this.transferArray[this.transferArray.length - 1]} 换乘指南:</strong><br>`;
        info += `<span>起点站出发:</span>${this.transferArray[0]}站<br>`;
        ...
        info += `搭乘<strong>${Array.from(name_set).toString()}</strong>,`;
        for (let i = 1; i < this.transferArray.length - 1; i++){
            let name_set_new = shMetro.getLineBetweenVertex(this.transferArray[i], this.transferArray[i + 1]);
            //到当前站点需要换乘
            if (!compareSet(name_set, name_set_new)) {
                info += `乘坐${count}站，抵达<strong>${this.transferArray[i]}站</strong><br>`;
                info += `换乘<strong>${Array.from(name_set_new).toString()}</strong>,`;
                count = 0;
            }
            count += 1;
            name_set = name_set_new;
        }
        info += `乘坐${count}站。<br>`;
        info += `<span>到达终点站:</span>${this.transferArray[this.transferArray.length - 1]}站`;

        return info;
    }

    //选择某条线路，将该线路的站点加入到lineArray中，并将当前线路对应的颜色返回
    selectLine(lineId) {
    ...
    loadData() {
        ...
        this.addNode('一大会址·黄陂南路', 33576877, 7569873, ['1号线']);
        ...
        this.addNode('龙阳路', 33586269, 7571481, ['2号线', '7号线', '16号线', '磁悬浮']);
        ...
        this.addNode('长江南路', 33578901, 7560696, ['3号线']);
        ...
        this.addNode('曹杨路', 33570683, 7568483, ['3号线', '4号线', '11号线(嘉定北-迪士尼)', '11号线(花桥-迪士尼)']);
        ...
        this.addNode('浦东大道', 33582009, 7568427, ['4号线']);
        ...
        this.addNode('大世界', 33577547, 7569501, ['8号线']);
        ...
        this.addNode('江浦路', 33581897, 7565492, ['8号线']);
        ...
        this.addNode('蓝天路', 33588534, 7568339, ['9号线']);
        this.addNode('芳甸路', 33586357, 7569109, ['9号线']);
        this.addNode('杨高中路', 33585270, 7569473, ['9号线']);
        ...
        this.addNode('国权路', 33580966, 7564289, ['10号线(航中路-基隆路)', '10号线(虹桥火车站-基隆路)']);
        ...
        this.addNode('豫园', 33578454, 7569449, ['10号线(航中路-基隆路)', '10号线(虹桥火车站-基隆路)']);
        ...
        this.addNode('真如', 33569516, 7567521, ['11号线(嘉定北-迪士尼)', '11号线(花桥-迪士尼)']);
        ...
        this.addNode('御桥', 33587748, 7575294, ['11号线(花桥-迪士尼)', '11号线(嘉定北-迪士尼)']);
        ...
        this.addNode('江浦公园', 33582489, 7566363, ['12号线']);
        ...
        this.addNode('武宁路', 33572109, 7568899, ['13号线']);
        ...
        this.addNode('莲溪路', 33587247, 7574373, ['13号线']);
        ...
        this.addNode('铜川路', 33568387, 7567523, ['15号线']);
