...
// g.attr("transform", "scale(1)"); //将图的初始大小设置为原大小
...
//图上一个节点的相关参数
const station_r = 8; //节点的半径
const station_fill = "white"; //节点的填充色
const station_stroke = "black"; //节点边框的颜色
...
function drawNode(name) {
    ...
    g.append("circle").attr("cx", xPos).attr("cy", yPos).attr("r", station_r)
...
function drawLineWithColor(station1, station2, color) {
    ...
    g.append("line").attr("x1", startX).attr("y1", startY)
...
function drawText(name) {
    ...
    g.append("text").text(station.name).attr("x", xPos + station.xSpace)
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        for (let connectedStation of node.edges.keys()) {
            ...
            if (arr.includes(connectedStation)) {
                //如果当前边已经被画过，则跳过
                if (paintedEdge.has(node.name + "-" + connectedStation)) {
                    continue;
                }

                drawLineWithColor(node.name, connectedStation, color);

                //将当前边添加到已经画过的边的集合中去
                paintedEdge.add(node.name + "-" + connectedStation);
                paintedEdge.add(connectedStation + "-" + node.name);
            }
        }

        drawNode(node.name); //画所有的节点
    }
...
function drawMap() {
    ...
    for (node of shMetro.vertexMap.values()) {
        ...
        for (let connectedStation of node.edges.keys()) {
            ...
            drawLine(node.name, connectedStation);
